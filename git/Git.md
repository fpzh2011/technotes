...
文件比较 `git diff readme.txt`。但是这样还是不如Eclipse等直观。
`git diff HEAD -- readme.txt` 可以比较工作区与版本库当前分支的差异。
...
Git中，用HEAD表示当前版本（不一定是最新版本），上一个版本是HEAD^，上上一个版本是HEAD^^，依次类推。往上100个版本，可以写为HEAD~100。
...
commit将暂存区的内容移动到版本库（Repository）的当前分支。文件提交后，暂存区里还是会暂存一下提交的文件（可以用`git ls-files --stage`查看暂存区的内容），直到下次“git add”时把它冲掉。
...
比较本地库与远程库的差异：`git diff HEAD origin/master`
...
创建分支 `git checkout -b dev` 。这条命令相当于以下两条命令：
